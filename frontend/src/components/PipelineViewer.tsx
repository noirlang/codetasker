...
export default function PipelineViewer({
  ...
  useEffect(() => {
    ...
    // Running / Streaming log simulator
    const triggerNextLine = () => {
      if (lineIndex < steps.length) {
        ...
        logTimer = setTimeout(() => {
          ...
          lineIndex++;
          triggerNextLine();
        }, step.delay / 2); // Accelerate simulation slightly
      }
    };
  ...
  }, [selectedNode, sha, message, normalizedRuns]);
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-[#2a2a2a] bg-[#0c0c0c] p-4 my-2 animate__animated animate__fadeIn" style={{ animationDuration: '0.2s' }}>
      {/* Node Graph Header */}
      <div className="flex items-center gap-2 border-b border-[#1e1e1e] pb-2">
        <Zap size={14} className="text-amber-400 animate-pulse" />
        <span className="text-[11px] font-semibold text-white tracking-wider uppercase font-mono">CI/CD Workflow Pipeline</span>
        <span className="text-[10px] text-[#666666] font-mono ml-auto">Visual Flow (n8n Engine style)</span>
      </div>

      {/* Node Graph Area */}
      <div 
        className="relative overflow-x-auto rounded-lg border border-[#1e1e1e] p-6 bg-[#090909] min-h-[140px] flex items-center justify-start gap-12 select-none"
        style={{
          backgroundImage: 'radial-gradient(#222222 1px, transparent 1px)',
          backgroundSize: '12px 12px',
        }}
      >
        {/* SVG overlay to draw connecting curves */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
          <defs>
            ...
            <linearGradient id="grad-failure" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="#10b981" stopOpacity="0.3" />
          ...
          <path d="M 125 70 C 150 70, 160 70, 185 70" stroke="#10b981" strokeWidth="1.5" fill="none" strokeDasharray="3 3" />
        ...
        <div 
          ...
          className={`relative z-10 w-28 shrink-0 rounded-md border p-2 cursor-pointer transition-all text-left ${getNodeBorder('trigger', 'completed', 'success')}`}
        >
        ...
        <div 
          ...
          className={`relative z-10 w-32 shrink-0 rounded-md border p-2 cursor-pointer transition-all text-left ${getNodeBorder('ci-init', 'completed', 'success')}`}
        >
          <div className="flex items-center gap-1.5">
        ...
        <div className="relative z-10 flex flex-col gap-3 shrink-0">
          {normalizedRuns.map((run, idx) => (
            <div
              ...
              className={`w-40 rounded-md border p-2 cursor-pointer transition-all text-left flex items-center justify-between gap-2 ${getNodeBorder(run.name, run.status, run.conclusion)}`}
            >
              <div className="min-w-0">
                <span className="text-[10px] font-bold text-white font-mono uppercase truncate block">{run.name}</span>
      ...
      <div className="rounded-lg border border-[#1e1e1e] bg-[#050505] flex flex-col overflow-hidden">
        ...
        {/* Console logs output */}
        <div className="p-3 font-mono text-[10px] h-[150px] overflow-y-auto flex flex-col gap-1 leading-5 text-left bg-black/90">
          {logs.map((log, idx) => {
