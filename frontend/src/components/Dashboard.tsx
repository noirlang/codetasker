...
import { appTokensApi, reposApi, systemApi } from '../api/client';
import type { Repo, ApiError, Organization, User, AppToken } from '../types';
...
function SettingsContent({
  ...
  const [rawTokenNotice, setRawTokenNotice] = useState<string | null>(null);
  const [copiedToken, setCopiedToken] = useState(false);

  const fetchAppTokens = async () => {
    setLoadingTokens(true);
    try {
      const list = await appTokensApi.list();
      setAppTokens(list);
    } catch {
      setAppTokens([]);
    } finally {
  ...
  const handleCreateToken = async (e: React.FormEvent) => {
    ...
    setRawTokenNotice(null);
    setCopiedToken(false);
    try {
      const res = await appTokensApi.create(newTokenName.trim());
      setRawTokenNotice(res.raw_token);
      setAppTokens((prev) => [res.app_token, ...prev]);
      setNewTokenName('');
    } catch {
      // failed
    } finally {
  ...
  const handleDeleteToken = async (tokenId: string) => {
    if (!confirm('Are you sure you want to revoke this App Token?')) return;
    setAppTokens((prev) => prev.filter((t) => t.id !== tokenId));
    try {
      await appTokensApi.delete(tokenId);
    } catch {
      fetchAppTokens();
    }
  };
